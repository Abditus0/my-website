...
This room is all about how data actually travels across a network. We already know data gets broken into smaller pieces, now we learn what those pieces actually are.
...
Simple rule to remember: if IP addresses are involved, you're talking about a **packet**. If you strip all that away, you're left with a **frame**.
...
- **Time to Live (TTL)**:
 ...
 every packet has a timer. If it never reaches its destination, it expires and gets removed so it doesn't clog up the network
...
**Question: What is the value of the flag given at the end of the conversation?** `THM{TCP_HANDSHAKE_COMPLETE}`
...
**Question: What is the flag received from the challenge?** `THM{PORTS_101_COMPLETE}`
